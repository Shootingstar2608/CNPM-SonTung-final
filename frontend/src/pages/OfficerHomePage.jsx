...
const OfficerHomePage = () => {
  ...
  useEffect(() => {
    // allow officer-like roles
    if (!['OFFICER', 'DEPARTMENT', 'UNIVERSITY_OFFICER'].includes(role)) {
  ...
  const avatarLabel = (localStorage.getItem('user_data') ? JSON.parse(localStorage.getItem('user_data')).name : 'O').charAt(0).toUpperCase();
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <header className="border-b bg-white sticky top-0 z-30">
        <div className="max-w-6xl mx-auto h-16 px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-5">
            ...
            {/* Thông tin chỉ hiện cho các role officer-like */}
            {['OFFICER','DEPARTMENT','UNIVERSITY_OFFICER'].includes(role) && (
              <button onClick={() => setInfoOpen(true)} className={`text-sm font-semibold py-1 text-gray-700 hover:text-blue-600`}>Thông tin</button>
            )}
            <button className={`text-sm font-semibold py-1 text-gray-700 hover:text-blue-600`}>Báo cáo</button>
            {/* 'Quản lý người dùng' removed from officer nav (admin-only). */}
          </nav>
      ...
      {/* Quick cards: show overview counts in styled cards */}
